...
[Unix Threads in C (Playlist)](https://youtube.com/playlist?list=PLfqABt5AS4FmuQf70psXrsMLEDQXNkLq2&si=5iUcoJWyEDH3TWx4)

[CPU Scheduling Basics (Playlist)](https://youtu.be/Jkmy2YLUbUY?si=Fho9G6wimwpsAxde)
...
AI was utilized as a collaborative tool throughout the development lifecycle to ensure robust concurrency and clean code architecture. Specifically:

-   **Architectural Design**: AI assisted in designing the Min-Heap structure used for the ```Earliest Deadline First (EDF)``` scheduler, providing guidance on heap property maintenance during ```heap_push``` and ```heap_pop``` operations

-   **Concurrency Debugging**: AI was used to analyze potential race conditions during the implementation of the ```monitor_routine``` and ```take_dongles``` logic, specifically ensuring that mutex locking sequences followed a strict hierarchy to prevent deadlock.

-   **Documentation**: AI aided in refining the explanations of thread synchronization primitives and the application of Coffman’s conditions within the context of this specific simulation.
